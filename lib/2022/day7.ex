defmodule Year2022.Day7 do
  ...
  defp part1(str),
    do:
      ...
      |> build_directory_map(%{"/" => []}, %{})
      |> to_size_map()
      |> find_total_size_max(100_000)

  defp build_directory_map(commands, dir_map, rev_map),
    do:
      commands
  ...
  defp to_directory_map([cmd | results], {current_dir, {dir_map, rev_map}}) do
    case cmd do
      "ls" ->
        {current_dir, Enum.reduce(results, {dir_map, rev_map}, &add_to_dir(&1, &2, current_dir))}

      "cd .." ->
        {Map.get(rev_map, current_dir), {dir_map, rev_map}}
  ...
  defp to_size_map({_, {map, _}}), do: to_size_map(%{}, "/", map) |> elem(0)
  ...
  defp to_size_map(size_map, dir, dir_map) do
    if Map.has_key?(size_map, dir) do
      ...
      case dirs do
        ...
        dirs ->
          {new_size_map, new_size} =
            Enum.reduce(dirs, {size_map, 0}, fn d, {sm, s} ->
  ...
  defp find_total_size_max(map, n),
    do: map |> Map.values() |> Enum.filter(fn x -> x <= n end) |> Enum.sum()
  ...
  defp part2(str) do
    ...
    min_size = 30_000_000 - (70_000_000 - Map.get(size_map, "/"))
