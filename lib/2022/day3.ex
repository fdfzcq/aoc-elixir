defmodule Year2022.Day3 do
  ...
  defp do_find_item_type({comp1, comp2}),
    do:
      comp1
      |> Enum.find(fn c -> Enum.member?(comp2, c) end)

  defp find_item_type(rucksack),
    do:
      rucksack
      |> String.to_charlist()
  ...
  defp find_group_badge_item_type([s1, s2, s3]),
    do: Enum.find(s1, fn c -> Enum.member?(s2, c) && Enum.member?(s3, c) end)
